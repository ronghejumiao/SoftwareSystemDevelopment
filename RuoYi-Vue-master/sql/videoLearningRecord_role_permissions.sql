-- 查询视频学习记录菜单ID
SELECT @menuId := menu_id FROM sys_menu WHERE perms = 'system:videoLearningRecord:list';
...
-- 查询查询权限菜单ID
SELECT @queryMenuId := menu_id FROM sys_menu WHERE perms = 'system:videoLearningRecord:query' AND parent_id = @menuId;
