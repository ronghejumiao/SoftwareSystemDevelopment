-- 查询视频资源菜单ID
SELECT @menuId := menu_id FROM sys_menu WHERE perms = 'system:videoresource:list';
...
-- 查询查询权限菜单ID
SELECT @queryMenuId := menu_id FROM sys_menu WHERE perms = 'system:videoresource:query' AND parent_id = @menuId;
