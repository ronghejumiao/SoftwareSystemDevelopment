<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="班级ID，关联class表" prop="classId">
        <el-input
          v-model="queryParams.classId"
          placeholder="请输入班级ID，关联class表"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="classList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="关联ID，主键，自增" align="center" prop="id" />
      <el-table-column label="学生ID，关联sys_user表" align="center" prop="studentId" />
      <el-table-column label="班级ID，关联class表" align="center" prop="classId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="班级ID，关联class表" prop="classId">
          <el-input v-model="form.classId" placeholder="请输入班级ID，关联class表" />
        </el-form-item>
        <el-form-item label="加入时间" prop="joinTime">
          <el-date-picker clearable
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        classId: [
          { required: true, message: "班级ID，关联class表不能为空", trigger: "blur" }
        ],
        joinTime: [
  ...
  methods: {
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加学生班级关联，实现学生与班级的多对多关系"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      ...
      getClass(id).then(response => {
        ...
        this.open = true
        this.title = "修改学生班级关联，实现学生与班级的多对多关系"
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除学生班级关联，实现学生与班级的多对多关系编号为"' + ids + '"的数据项？').then(function() {
        return delClass(ids)
      }).then(() => {
        this.getList()
