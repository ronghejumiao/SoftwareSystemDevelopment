<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="概念图名称" prop="mapName">
        <el-input
          v-model="queryParams.mapName"
          placeholder="请输入概念图名称"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="imgList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="概念图ID，主键，自增" align="center" prop="mapId" />
      <el-table-column label="课程ID，关联course表" align="center" prop="courseId" />
      <el-table-column label="概念图名称" align="center" prop="mapName" />
      <el-table-column label="概念图URL" align="center" prop="mapUrl" />
    ...
    <!-- 添加或修改课程概念图，存储课程的概念图URL（1对多关系）对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="概念图名称" prop="mapName">
          <el-input v-model="form.mapName" placeholder="请输入概念图名称" />
        </el-form-item>
        <el-form-item label="概念图URL" prop="mapUrl">
          <el-input v-model="form.mapUrl" type="textarea" placeholder="请输入内容" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        mapName: [
          { required: true, message: "概念图名称不能为空", trigger: "blur" }
        ],
        mapUrl: [
          { required: true, message: "概念图URL不能为空", trigger: "blur" }
        ],
        createTime: [
  ...
  methods: {
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加课程概念图，存储课程的概念图URL（1对多关系）"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      const mapId = row.mapId || this.ids
      getImg(mapId).then(response => {
        this.form = response.data
        this.open = true
        this.title = "修改课程概念图，存储课程的概念图URL（1对多关系）"
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除课程概念图，存储课程的概念图URL（1对多关系）编号为"' + mapIds + '"的数据项？').then(function() {
        return delImg(mapIds)
      }).then(() => {
        this.getList()
